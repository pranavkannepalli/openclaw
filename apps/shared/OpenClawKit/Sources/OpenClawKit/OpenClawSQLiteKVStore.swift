...
public enum OpenClawSQLiteKVStore {
    ...
    public static func storeLocationForDisplay(scope: String, key: String) -> String {
        "\(self.databaseURL().path)#kv/\(scope)/\(key)"
    }

    public static func readString(scope: String, key: String) -> String? {
        guard let data = self.readJSONData(scope: scope, key: key) else { return nil }
        return try? JSONDecoder().decode(String.self, from: data)
    }

    public static func writeString(scope: String, key: String, value: String) throws {
        let data = try JSONEncoder().encode(value)
        try self.writeJSONData(scope: scope, key: key, data: data)
    }

    public static func readJSONData(scope: String, key: String) -> Data? {
        do {
            ...
            if status == SQLITE_ROW, let rawText = sqlite3_column_text(statement, 0) {
                ...
                return valueJSON.data(using: .utf8)
            }
    ...
    public static func writeJSONData(scope: String, key: String, data: Data) throws {
        guard let valueJSON = String(data: data, encoding: .utf8) else {
            throw NSError(
                domain: "OpenClawSQLiteKV",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: "SQLite KV value must be UTF-8 JSON"])
        }
        ...
        do {
            ...
            self.bindText(statement, index: 1, value: scope)
            self.bindText(statement, index: 2, value: key)
            self.bindText(statement, index: 3, value: valueJSON)
