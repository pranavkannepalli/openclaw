...
describe("state migrations", () => {
  ...
  it("runs legacy state migrations and canonicalizes the merged session store", async () => {
    ...
    const firstMessage = transcriptEvents[1] as { id?: unknown; parentId?: unknown };
    const secondMessage = transcriptEvents[2] as {
      id?: unknown;
      parentId?: unknown;
      message?: { role?: unknown };
    };
    const compaction = transcriptEvents[3] as {
      parentId?: unknown;
      firstKeptEntryId?: unknown;
      firstKeptEntryIndex?: unknown;
    };
    expect(firstMessage.id).toEqual(expect.any(String));
    expect(firstMessage.parentId).toBeNull();
    expect(secondMessage.parentId).toBe(firstMessage.id);
    expect(secondMessage.message?.role).toBe("custom");
